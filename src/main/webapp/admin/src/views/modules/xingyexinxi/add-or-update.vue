<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="行业名称" prop="xingyemingcheng">
          <el-input v-model="ruleForm.xingyemingcheng" 
              placeholder="行业名称" clearable  :readonly="ro.xingyemingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="行业名称" prop="xingyemingcheng">
              <el-input v-model="ruleForm.xingyemingcheng" 
                placeholder="行业名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                              </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='xingyemingcheng'){
            this.ruleForm.xingyemingcheng = obj[o];
	    this.ro.xingyemingcheng = true;
            continue;
          }
	                    }
                              }
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `xingyexinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xingyexinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
